...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    console.error('[dashboard/photos/[id] PUT]', err)
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { id } = await params
    await sql`DELETE FROM wedding_photos WHERE id = ${id} AND wedding_id = ${session.weddingId}`
    const config = await getWeddingConfigById(session.weddingId)
    if (config) revalidateWeddingPages(config.locales)
    return NextResponse.json({ ok: true })
  } catch (err) {
    console.error('[dashboard/photos/[id] DELETE]', err)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
