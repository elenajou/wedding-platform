...
export async function GET() {
  ...
  try {
    ...
    console.error('[dashboard/photos GET]', err)
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const rows = await sql`
      ...
      VALUES (${session.weddingId}, ${body.sort_order ?? 0}, ${body.src}, ${body.alt}, ${body.caption ?? null})
      RETURNING *
    `
    ...
    console.error('[dashboard/photos POST]', err)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
