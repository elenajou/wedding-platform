...
import DressCodeTab from '@/app/dashboard/_tabs/DressCodeTab'

type Props = { params: Promise<{ tab: string }> }

const BASE_TABS = ['tables', 'groups', 'guests']
...
export default async function DashboardTabPage({ params }: Props) {
  ...
  const TAB_LABELS: Record<string, string> = {
    ...
    dresscode: dt('tabDressCode', locale),
  }
  ...
  const allowedTabs = [
    ...
    ...(config.features.dressCode ? ['dresscode'] : []),
    'theme',
    'help',
  ]
  ...
  if (tab === 'tables') {
    ...
    content = <GuestsTab initialItems={guestRows as any[]} groups={groupRows as any[]} locale={locale} locales={config.locales} />
  } else if (tab === 'rsvps') {
    const rows = await sql`SELECT r.*, ig.name AS group_name, ig.allocated_seats, ig.table_name FROM rsvps r LEFT JOIN invitation_groups ig ON ig.id = r.group_id WHERE r.wedding_id = ${wid} ORDER BY r.created_at DESC`
  ...
  } else if (tab === 'dresscode') {
    const rows = await sql`SELECT * FROM wedding_dress_code WHERE wedding_id = ${wid} ORDER BY sort_order`
    content = <DressCodeTab initialItems={rows as any[]} locales={config.locales} defaultLocale={config.defaultLocale} locale={locale} />
  } else if (tab === 'help') {
    content = <HelpTab />
  } else if (tab === 'theme') {
    const [sectionRows, detailRows, heroElementRows, scheduleRows] = await Promise.all([
      ...
      sql`SELECT * FROM wedding_schedule WHERE wedding_id = ${wid} ORDER BY sort_order`,
    ])
    ...
    content = (
      <ThemeTab
        ...
        initialWeddingDetails={detailRows[0] as any ?? null}
        locale={locale}
        initialHeroElements={heroElementRows as any[]}
        locales={config.locales}
        defaultLocale={config.defaultLocale}
        initialScheduleItems={config.features.schedule ? scheduleRows as any[] : []}
      />
    )
  }
  ...
  return (
    <main style={{ minHeight: '100svh', background: '#faf7f2', fontFamily: "'EB Garamond', serif" }}>
      ...
      <div style={{ padding: '2rem', ...(tab !== 'theme' ? { maxWidth: 860, margin: '0 auto' } : {}) }}>
        {content}
      </div>
    </main>
  )
