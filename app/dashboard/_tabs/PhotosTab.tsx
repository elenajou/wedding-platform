...
type Props = { initialItems: Photo[]; locale?: string }
...
export default function PhotosTab({ initialItems, locale }: Props) {
  ...
  async function handleDelete(id: string) {
    if (!confirm(T('photosDeleteConfirm'))) return
  ...
  return (
    <div>
      <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, fontStyle: 'italic', fontWeight: 300, color: '#201d19', marginBottom: '1.5rem' }}>{T('photosTitle')}</p>
      ...
      <form onSubmit={handleAdd} style={{ display: 'grid', gridTemplateColumns: '0.4fr 3fr 2fr 2fr auto', gap: 8, marginBottom: '2rem', alignItems: 'flex-end' }}>
        {([['#', 'sort_order', '0'], [T('photosUrl'), 'src', T('photosUrlPh')], [T('photosAlt'), 'alt', T('photosAltPh')], [T('photosCaption'), 'caption', T('photosCaptionPh')]] as [string,string,string][]).map(([lbl, k, ph]) => (
          <div key={k}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr>
            <th style={th}>#</th>
            <th style={th}>{T('photosPreview')}</th>
            <th style={th}>URL</th>
            <th style={th}>{T('photosAlt')}</th>
            <th style={th}>{T('photosCaption')}</th>
            <th style={th}></th>
          </tr>
        </thead>
        <tbody>
          {items.map(item => editing?.id === item.id ? (
            ...
            <tr key={item.id}>
              ...
              <td style={{ ...td, whiteSpace: 'nowrap' }}>
                <button onClick={() => setEditing({ id: item.id, sort_order: String(item.sort_order), src: item.src, alt: item.alt, caption: item.caption ?? '' })} style={{ ...btn('#e8e0d4'), marginRight: 6 }}>{T('edit')}</button>
                <button onClick={() => handleDelete(item.id)} style={btn('#f5ebe8')}>{T('delete')}</button>
              </td>
            </tr>
          ))}
          {items.length === 0 && <tr><td colSpan={6} style={{ ...td, color: '#7a6e5f', fontStyle: 'italic' }}>{T('photosEmpty')}</td></tr>}
        </tbody>
      </table>
    </div>
  )
