...
export default function RsvpsTab({ initialItems }: Props) {
  ...
  return (
    <div>
      <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, fontStyle: 'italic', fontWeight: 300, color: '#201d19', marginBottom: '0.5rem' }}>Confirmaciones</p>

      <div style={{ display: 'flex', gap: '2rem', marginBottom: '2rem' }}>
        {[['Total', items.length], ['Asistirán', attending.length], ['Invitados', totalAttending], ['Declinaron', declined.length]].map(([lbl, val]) => (
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr><th style={th}>Grupo</th><th style={th}>Estado</th><th style={th}>Invitados</th><th style={th}>Enviado por</th><th style={th}>Mensaje</th><th style={th}></th></tr></thead>
          <tbody>
            {items.map(item => (
              <tr key={item.id}>
                ...
                <td style={td}>
                  <span style={{ padding: '2px 8px', borderRadius: 10, fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', background: item.attending ? '#e8f4ec' : '#fce8e4', color: item.attending ? '#2d6a40' : '#c4614a' }}>
                    {item.attending ? 'Asistirá' : 'Declinó'}
                  </span>
                </td>
                ...
                <td style={td}><button onClick={() => handleDelete(item.id)} style={{ padding: '4px 10px', background: '#f5ebe8', color: '#201d19', border: 'none', fontFamily: "'EB Garamond', serif", fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', cursor: 'pointer', borderRadius: 1 }}>Eliminar</button></td>
              </tr>
            ))}
            {items.length === 0 && <tr><td colSpan={6} style={{ ...td, color: '#7a6e5f', fontStyle: 'italic' }}>Sin confirmaciones aún.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
