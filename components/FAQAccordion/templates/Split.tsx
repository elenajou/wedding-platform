...
export default function Split({ lang, dict }: FAQTemplateProps) {
      ...
      <div style={{ maxWidth: 760, margin: '0 auto' }}>
        {dict.items.map((item, i) => (
          <div key={i} style={{
            ...
            borderBottom: '0.5px solid var(--color-border)', padding: '1.4rem 0',
          }}
          className="faq-split-row"
          >
            <style>{`.faq-split-row{grid-template-columns:1fr 1fr}@media(max-width:540px){.faq-split-row{grid-template-columns:1fr!important}}`}</style>
            <p style={{ fontSize: 17, fontStyle: 'italic', color: 'var(--color-text)', paddingRight: '1rem', borderRight: '0.5px solid var(--color-border)', marginRight: '-0.5px' }}>
              {item.question}
            </p>
